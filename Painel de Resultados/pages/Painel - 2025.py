...
######### ESTRUTURA DE SELEÇÃO DO MÊS-------------------------------------------------------------------------------------------
    ...
    opcao_select1= st.selectbox( # cria caixa de seleção por mês na sidebar
     "Seleciona o Mês",
    ("Janeiro", "Fevereiro", "Março","Abril","Maio","Junho","Julho","Agosto","Setembro",
     "Outubro","Novembro","Dezembro"))
     
    if opcao_select1 =="Abril":
        st.text (f"Movimentação do mês de {opcao_select1}")      
#-BLOCO 13 RECEBE TOTAL DO MÊS FILTRADO POR MÊS ESPECIFICO ABRIL-------------------------------------------------------------      
        ...
        col7.markdown(f'<div class="caixa-borda">Vendas<br>{vendas_formatadas}</div>', unsafe_allow_html=True)
        col8.markdown(f'<div class="caixa-borda">Compras<br>{compras_formatadas}</div>', unsafe_allow_html=True)
        col9.markdown(f'<div class="caixa-borda">Diferença<br>{diferenca_formatada}</div>', unsafe_allow_html=True)
        col10.markdown(f'<div class="caixa-borda">% Compras/Vendas<br>{round(percentual_compras_vendas_mes, 2)}%</div>', unsafe_allow_html=True)
...
#-BLOCO 16- EXIBINDO TOTAIS POR GRUPO DENTRO DO MES ESCOLHIDO------------------------------------------------------------------- 
        ...
        df_baseRk01 = pd.read_excel(r'C:\Painel de Resultados\Bases\Ranking\Rk_Lj01_04_2024.xlsx', thousands=".", index_col=None)
        #df_filtrado_por_mes = df_baseRk01[df_baseRk01['mes_resumido'] == 'Abr']
        df_filtrado_por_mes = df_baseRk01[df_baseRk01['mes_resumido'] == mes_especifico]
        #mes_especifico 
        #st.dataframe(df_baseRk01)
        df_baseRk01['Resultado'] = df_baseRk01['custogerencial'] * df_baseRk01['Quantidade Vendida']
        df_top10 = df_filtrado_por_mes.nlargest(10, 'Valor da Venda')
        df_top10['Ranking'] = range(1, len(df_top10) + 1)
        
       
        df_top10['Valor da Venda'] = df_top10['Valor da Venda'].apply(formatar_moeda)
        
        colunas_exibir = ['Descrição do produto', 'Ranking', 'Valor da Venda']
        st.subheader(f"Ranking 10 Mais Vendidos de Abril") # título de totalizadores
        st.dataframe(df_top10[colunas_exibir], hide_index=True)
